...
class ColorFont(object):
    ...
    def __init__(self, filename=None):
        super(ColorFont, self).__init__()
        self.__anim = Animation()
        self.char_size = None
        self.bitmaps = [None,None,None,None,None,None,None,None,None,None,None,None,None,None,None,None]
        if filename != None:
            self.load(filename)
    ...
    def load(self, filename):
        ...
        self.bitmaps[0] = self.__anim.frames[0]
        self.char_widths = []
        for i in range(96):
    ...
    def save(self, filename):
        ...
        out.frames = [self.bitmaps[0], Frame(out.width, out.height)]
        for i in range(96):
            out.frames[1].set_dot(i%self.__anim.width, i/self.__anim.width, self.char_widths[i])
        out.save(filename)

    def draw(self, frame, text, x, y,color=0):
        ...
        for ch in text:
            ...
            Frame.copy_rect(dst=frame, dst_x=x, dst_y=y, src=self.bitmaps[color], src_x=char_x, src_y=char_y, width=width, height=self.char_size, op=self.composite_op)
