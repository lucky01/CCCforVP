...
class SkillShot(ep.EP_Mode):
    ...
    def sw_flipperLwL_active(self,sw):
        if self.selecting and self.game.switches.shooterLane.is_active():
            ...
            if self.rcount == 8: #change 5 to 8 4-2-14
                self.lcount += 1
                if self.lcount == 2:
    ...
    def generate_prizes(self):
        ...
        # here's the super skill shot prizes
            ...
            if self.game.user_settings['Gameplay (Feature)']['Tribute Mode'] == "Enabled":
                prizes.append("W") #4-2-14
                prizes.append("M") #4-2-14
                prizes.append("S") #4-2-14
                prizes.append("V")
        ...
        if len(prizes) >= (8 - count): #4-2 change 5 to 8
            remove = True
        else:
            remove = False
        while count < 8:   #4-2-14 changed 5 to 8 items
            item = random.randrange(len(prizes))
            self.selectedPrizes += prizes[item]
            if remove:
                prizes.remove(prizes[item])
            else:
                if prizes[item] == "D" : #change code "J" or prizes[item] == "W":
        ...
        #4-2-14 changed 4:5 to 7:8 to show 8 prizes
        self.selectedPrizes = self.selectedPrizes[7:8] + self.selectedPrizes
    ...
    def enable_selecting(self):
        ...
        if self.game.user_settings['Gameplay (Feature)']['Tribute Mode'] == "Enabled":
            ...
            self.choices.append("S") #4-2
            self.choices.append("W") #4-2
            self.choices.append("M") #4-2
        self.change_prizes()
    ...
    def sw_skillBowl_active(self, sw):
        ...
        if self.super:
            ...
            if self.active == 8: #4-2-14 change 5 to 8
                self.active = 1
        ...
        prizeTemp1 = self.selectedPrizes[0:4] #changed 0:4 to 0:7 to add 3 more
        prizeTemp2 = self.selectedPrizes[4:5] #4-14 change 4:5 to 4:8 to  add 3 more
    ...
    def skillshot_set(self):
        ...
        self.live = True
        if self.live == True:
        #if not self.super:    replaced W, J L to start/shoot
            self.skillshot_award()
        else:
            # if we're doing tribute - bump the shot over if it was on the loop
            if self.selectedPrizes[8:] == "V" and self.active == 1:      #change 5 to 8 4-2-14
                self.active = 2
    ...
    def skillshot_award(self,switch=0):
        ...
        self.stop_music()
        # play the sound
        if self.selectedPrizes[8:] == "V": #change 5 to 8 4-2-14
        ...
        # award the prize -
        if self.selectedPrizes[8:] == "C": #change 5 to 8 4-2-14
            self.game.score(10)
        ...
        elif self.selectedPrizes[8:] == "D": #change 5 to 8 4-2-14
            self.game.score(20)
        ...
        elif self.selectedPrizes[8:] == "E": #chage 5 to 8 4-2-14
            # this one is the bounty
            self.layer = None
            self.game.saloon.light_bounty(self.start_gameplay)
            return

        elif self.selectedPrizes[8:] == "F": #change 5 to 8 4-2-14
        ...
        elif self.selectedPrizes[8:] == "G": #change 5 to 8 4-2-14
        ...
        elif self.selectedPrizes[8:] == "H": #change 5 to 8 4-2-14
        ...
        elif self.selectedPrizes[8:] == "I": #change 5 to 8 4-2-14
        ...
        elif self.selectedPrizes[8:] == "J": #change 5 to 8 4-2-14
            # audit
        ...
        elif self.selectedPrizes[8:] == "K": #change 5 to 8 4-2-14
        ...
        # new 100 grand award
        elif self.selectedPrizes[8:] == "X": #change 5 to 8 4-2-14
            awardStringTop = ep.format_score(100000)
            awardStringBottom = "POINTS"
            self.game.score(100000)

        # the 1 million prize
        elif self.selectedPrizes[8:] == "N": #change 5 to 8 4-2-14
        ...
        # super prizes
        # three million
        elif self.selectedPrizes[8:] == "O": #change 5 to 8 4-2-14
            ...
            return
        # bonus x
        elif self.selectedPrizes[8:] == "P": #change 5 to 8 4-2-14
            self.game.score(1000,bonus=True)
            awardStringTop = "BONUS X"
            ...
            self.kick = True
        # gunfight
        elif self.selectedPrizes[8:] == "Q": #change 5 to 8 4-2-14
        ...
        # drunk multiball
        elif self.selectedPrizes[8:] == "R": #change 5 to 8 4-2-14
            self.layer = None
            self.game.score(50000)
            self.game.add_bonus(1200)
            # light the dmb
        ...
        # move your train
        elif self.selectedPrizes[8:] == "S": #change 5 to 8 4-2-14
            awardStringTop = "MOVE"
        ...
        # cva
        elif self.selectedPrizes[8:] == "T":#change 5 to 8 4-2-14
        ...
        # marshall multiball
        elif self.selectedPrizes[8:] == "U":#change 5 to 8 4-2-14
        ...
        # Tribute mode
        elif self.selectedPrizes[8:] == "V":#change 5 to 8 4-2-14
        ...
        # franks n beans
        elif self.selectedPrizes[8:] == "W": #change 5 to 8 4-2-14
        ...
        #print "SkillShot Awarded " + str(self.selectedPrizes[8:]) + ": " + str(awardStringTop) + " " + str(awardStringBottom)#change 5 to 8 4-2-14
        self.award_display(awardStringTop,awardStringBottom)

    def award_display(self,awardStringTop,awardStringBottom,start=True):
        # the award icon
        prizeList = dmd.TextLayer(126, 1,
        self.game.assets.font_skillshot, "right", opaque=False).set_text(self.selectedPrizes[8:])#change 5 to 8 4-2-14
        # the award text
    ...
    def super_hit(self,made=None,switch=0):
        ...
        if made:
            ...
            if self.selectedPrizes[8:] == "T" or self.selectedPrizes[8:] == "U":#change 5 to 8 4-2-14
                pass
            else:
